...
class Text_Temp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Text(
      BlocProvider.of<GetWeatherCubit>(context).weatherModel!.temp.toString(),
...
class Text_updated extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    ModelWeather get_updated_cubit =
        BlocProvider.of<GetWeatherCubit>(context).weatherModel!;
    return Text(
        "updated ${get_updated_cubit.date.hour}:${get_updated_cubit.date.minute}",
        style: TextStyle(
          fontSize: 20,
        ));
  }
